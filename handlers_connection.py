"""Connection management for Notarize Connector."""
...
from notarize_client import NotarizeClient

_SECRET = "notarize_connections"
...
async def resolve_client(ctx, connection_id: str = "") -> NotarizeClient:
    conns = await _load_connections(ctx)
    if not conns:
        raise ValueError("No Notarize connections configured. Use connect_notarize first.")
    conn = None
    if connection_id:
        for c in conns:
        ...
        if not conn:
            conn = conns[0]
    return NotarizeClient(api_key=conn["api_key"], base_url=conn.get("base_url", ""))

@chat.function(
    "connect_notarize",
    "Connect Notarize account via credentials.",
    action_type="write",
    chain_callable=True,
    event="notarize-connector.connect_notarize",
    effects=["create:connection"],
    data_model=ConnectionRecord
)
async def connect_notarize(ctx, params: ConnectParams) -> ActionResult[ConnectionRecord]:
    client = NotarizeClient(api_key=params.api_key, base_url=params.base_url)
    res = await client.verify_auth()
    if res.get("status") == "error":
        return ActionResult.error(f"Failed to authenticate with Notarize: {res.get('error')}")

    conns = await _load_connections(ctx)
    cid = f"conn_{uuid.uuid4().hex[:8]}"
    record = {
        "id": cid,
        "label": params.label or "Primary Notarize",
...
@chat.function(
    "list_connections",
    "List configured Notarize connections.",
    action_type="read",
    chain_callable=True,
    event="notarize-connector.list_connections",
...
@chat.function(
    "disconnect_notarize",
    "Disconnect Notarize account and delete stored credentials.",
    action_type="destructive",
    chain_callable=True,
    event="notarize-connector.disconnect_notarize",
    effects=["delete:connection"],
    data_model=DeleteResult
)
async def disconnect_notarize(ctx, params: ConnectionIdParams) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
    if not conns:
