"""Resource handlers for Notarize Connector."""
...
@chat.function(
    "list_agreements",
    "List agreements or signature requests in Notarize.",
    action_type="read",
    chain_callable=True,
    event="notarize-connector.list_agreements",
...
async def list_agreements(ctx, params: ListAgreementsParams) -> ActionResult[AgreementList]:
    try:
        ...
        return ActionResult.success(AgreementList(agreements=agreements, total=len(agreements)), summary=f"Retrieved {len(agreements)} agreements from Notarize.")
    except Exception as e:
        return ActionResult.error(f"Error listing agreements: {e}")

@chat.function(
    "get_agreement",
    "Get details of one agreement or signature request in Notarize.",
    action_type="read",
    chain_callable=True,
    event="notarize-connector.get_agreement",
...
@chat.function(
    ...
    "Send a new document for signature via Notarize.",
    action_type="write",
    chain_callable=True,
    event="notarize-connector.create_signature_request",
...
@chat.function(
    ...
    "Cancel or void an in-flight signature request in Notarize.",
    action_type="destructive",
    chain_callable=True,
    event="notarize-connector.cancel_signature_request",
...
@chat.function(
    "audit_esign_health",
    "Audit in-flight signature requests, turnaround rates, and expired agreements in Notarize.",
    action_type="read",
    chain_callable=True,
    event="notarize-connector.audit_esign_health",
    effects=["read:audit"],
...
async def audit_esign_health(ctx, params: NoParams) -> ActionResult[ESignAuditReport]:
    try:
        ...
        rep = ESignAuditReport(
            ...
            summary=f"Notarize health audit: {len(agreements)} total tracked agreements, {pending} pending signatures, {completed} completed."
        )
