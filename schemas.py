"""Pydantic schemas for Notarize Connector (C34. E-Signature & Agreement Management)."""
...
class ConnectParams(BaseModel):
    label: str = Field(default="", description="Friendly connection label, e.g. Primary Notarize.")
    api_key: str = Field(description="Notarize (Proof) API Key / Organization Token.")
    base_url: str = Field(default="https://api.notarize.com/v1", description="Notarize API base URL.")
...
class GetAgreementParams(BaseModel):
    connection_id: str = Field(default="", description="Optional connection ID.")
    agreement_id: str = Field(description="Notarize agreement or signature request ID.")
